 ...
 * under the tnylpo CP/M emulator).  Reads BASE.hex and writes BASE.com -- the
 * absolute memory image from the lowest to the highest loaded address, padded
 * up to a 128-byte CP/M record boundary with 0x00 -- and prints the same
 * report and the same diagnostics as the original.
 *
 * This is a standalone program (it does not use the assembler engine).
 */
...
static int rd_byte(FILE *f, int *ok)
{
    int c1, c2, hi, lo;
    c1 = fgetc(f);
    if (c1 == EOF) { *ok = 0; return 0; }   /* don't read past EOF */
    c2 = fgetc(f);
    hi = hexval(c1);
    lo = hexval(c2);
    if (hi < 0 || lo < 0) { *ok = 0; return 0; }
    *ok = 1;
    return (hi << 4) | lo;
}
...
/* Dump a failing record's bytes the way the original does: a "<addr>: " header
 * line, then the bytes 16 per line each prefixed with their address. */
static void dump_record(unsigned recaddr, const unsigned char *data, int n)
{
    int i;
    (void)printf("%04X: \n", recaddr);
    for (i = 0; i < n; i++) {
        ...
        if ((i + 1) % 16 == 0) (void)printf("\n");
    }
    if (n % 16 != 0) (void)printf("\n");
}

/******************************************************************************/

/* Every "ERROR: " diagnostic goes through the original's common routine, which
 * prints  "ERROR: <msg>" CRLF "LOAD  ADDRESS <addr>"  with no trailing newline
 * (verified by disassembly at 0x0412 in orig/hexcom.com). */
static void fatal_load(const char *msg, unsigned addr)
{
...
static void record_error(const char *msg, unsigned recaddr, unsigned erraddr,
...
int main(int argc, char **argv)
{
    ...
    if (src == NULL) fatal_load("CANNOT OPEN SOURCE FILE", TPA);

    /* The original creates the output file before reading, so a malformed HEX
     * leaves an empty .com behind; opening it here reproduces that. */
    out = fopen(dstname, "wb");
    if (out == NULL) fatal_load("DIRECTORY FULL", TPA);

    for (;;) {
        unsigned addr;
        int c, ok, ll, tt, i, sum, cc;
        do { c = fgetc(src); } while (c != ':' && c != EOF && c != 0x1A);
        if (c != ':') break;        /* end of input: no further records */

        ll = rd_byte(src, &ok);
        if (!ok) record_error("INVALID HEX DIGIT", 0, 0, data, 0);
        addr = (unsigned)rd_byte(src, &ok) << 8;
        if (!ok) record_error("INVALID HEX DIGIT", 0, 0, data, 0);
        addr |= (unsigned)rd_byte(src, &ok);
        if (!ok) record_error("INVALID HEX DIGIT", 0, 0, data, 0);
        tt = rd_byte(src, &ok);
        if (!ok) record_error("INVALID HEX DIGIT", addr, addr, data, 0);

        if (tt == 0x01) break;      /* end-of-file record */
        if (tt != 0x00) continue;   /* ignore other record types */

        /* A zero-length record (e.g. the ":0000000000" terminator some HEX
         * writers emit) loads nothing: it neither triggers the < 100 check nor
         * extends the image. */
        if (ll > 0 && addr < TPA)
            fatal_load("LOAD ADDRESS LESS THAN 100", addr);
        if (ll > 0 && !have_first) { first_addr = addr; have_first = 1; }

        sum = ll + (int)((addr >> 8) & 0xFF) + (int)(addr & 0xFF) + tt;
        for (i = 0; i < ll; i++) {
            int b = rd_byte(src, &ok);
            if (!ok) record_error("INVALID HEX DIGIT", addr,
    ...
    if (!have_first) { first_addr = 0; last_addr = 0; }
    ...
    if (fclose(out) != 0) fatal_load("CANNOT CLOSE FILE", first_addr);
    return 0;
}
