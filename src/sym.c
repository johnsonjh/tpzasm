...
/* The Ch lint cat's all sources into one unit; ci_eq also lives in assemble.c,
 * so rename this copy to avoid a clash. */
#ifdef _CH_
# undef  ci_eq
# define ci_eq sym_ci_eq
#endif
...
{
    unsigned h = 5381u;
    while (*s) h = h * 33u + (unsigned char)toupper((unsigned char)*s++);
    return h;
}
...
symtab *sym_new(void)
{
    ...
    if (t->bucket == NULL) { free(t); return NULL; }
    for (i = 0; i < t->nbuckets; i++) t->bucket[i] = NULL;
    return t;
}
...
symbol *sym_lookup(const symtab *t, const char *name)
{
    symbol *s;
    if (t == NULL) return NULL;
    for (s = t->bucket[hash(name) % (unsigned)t->nbuckets];
         s != NULL; s = s->next)
        if (ci_eq(s->name, name)) return s;
    return NULL;
}
...
void sym_collect(const symtab *t, symbol **buf)
{
    int i, n = 0;
    symbol *s;
    if (t == NULL) return;
    for (i = 0; i < t->nbuckets; i++)
        for (s = t->bucket[i]; s != NULL; s = s->next) buf[n++] = s;
}
