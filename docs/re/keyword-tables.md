# TDL / PSA assembler keyword tables — reverse-engineered

Decoded 2026-06-16 from the original CP/M `.COM` binaries in `orig/` of the
TPZASM project. Covers `zasm.com` (TDL Z80 v2.21), `pasm.com` (PSA PASM 1.02),
and `pasm2.com` (PSA PASM 2.00G). All three share one keyword-table format.
...
Table locations: zasm.com @ `0x2a02` (239 recs), pasm.com @ `0x2fba` (242 recs), pasm2.com @ `0x40c1` (350 recs).

Keyword counts: **zasm.com 2.21 = 239**, **pasm.com 1.02 = 242**, **pasm2.com 2.00G = 311** (= 242 + 69 pasm2-only). The unified table holds the 8080 set, the TDL Z80 extensions,
the standard Zilog set (pasm2), all directives, the Intel/M80 pseudo-ops
(pasm2 `.EPOP`), and operand keywords (`A`..`L`,`M`,`PSW`,`SP`,`X`,`Y`,`$`,`.`).

## zasm.com (TDL Z80 v2.21) — 239 keywords
...
**In zasm.com 2.21 but not pasm.com 1.02:** []

## Gap analysis — clone vs the originals it emulates

Our clone targets **byte-exact zasm.com 2.21 (`-z`) and pasm.com 1.02 (`-p`)**.
Comparing the decoded oracle tables against the clone's recognized keywords
(`src/insn.c` + `src/assemble.c`), matched on 6-char prefixes:

### ZASM 2.21 (`-z`): clone missing 0 (excluding register/segment/LC tokens it parses specially)
  *(none — full coverage)*

### PASM 1.02 (`-p`): clone missing 2 (excluding register/segment/LC tokens it parses specially)
  .GOTO, .PROGI

### Clone accepts these, but neither zasm.com 2.21 nor pasm.com 1.02 has them:
  .COMMO, .DB, .DC, .DS, .DW, .EXTRN, .NAME, .ORG, .PRINT, .PUBLI, .REQUE, .SUBTT, COMMON, DB, DC, DCS, DEFB, DEFW, DS, DSW, DW, END, EXTRN, ORG, PUBLIC

> **Fidelity note:** `DB DW DS DEFB DEFW DS ORG END EXTRN` (and the `.DB/.DW/
> .DS/.ORG` synonyms) are **pasm2 `.EPOP` / Intel-M80 forms** — they are NOT in
...
> The clone accepts them (assemble.c:5474-5859), so on a source using Intel
> forms the clone diverges from the originals (it assembles; they error).
> Correct fix for strict parity: gate these synonyms behind a future pasm2/
> `.EPOP` mode. No corpus fixture currently exercises this. See
> `asm-clone-pasm2-zop-dialect` memory.
