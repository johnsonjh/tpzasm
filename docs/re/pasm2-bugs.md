...
PASM 2.00G (`[C12011-0200G]`, 06/27/81) is a buggy beta of the PSA Macro
Assembler.  These are defects in `pasm2.com` itself, observed via the tnylpo
oracle.  **TPZASM does NOT reproduce these bugs** — the clone produces the
correct output.  Where a bug interferes with differential testing, the test
fixtures are shaped to avoid triggering it (noted per item).
...
**Clone behavior.**  TPZASM writes the complete assembled object for ANY size
(e.g. the 5-byte `LD A,1`/`NOP` `.XLINK` program emits `:030100003E0100BD` +
EOF; the 191-byte `intcond`-style program emits all of its records).  The clone
...
table is omitted from the listing (1.02 always emits it).  See the
`asm-clone-pasm2-findings` memory.  The clone always emits the symbol table.

## 3. Dormant/unfinished beta artifacts

The unfilled `XX/XX/81 XX:XX:XX` date template (no RTC under CP/M 2.2), the
unwired ` (OPCD) `/` (MACR) ` strings, and the `G` revision suffix are leftover
beta scaffolding that never surfaces in output.

## Notes on intentional clone/oracle differences (NOT bugs)

- **Binary `.PBIN` record chunking.**  2.00G packs binary data records into
  larger chunks than PASM 1.02; the clone uses the 1.02 chunking.  Identical
  content, different framing.  Real sources select `.PHEX` (ASCII), which is
  byte-exact, so `tools/vpasm2.sh` differences only the ASCII object.
- **Multi-character `.BYTE` `Q` error.**  pasm2 flags a multi-character TDL
  ...
  low byte); the clone emits the same byte without the `Q`.  Error-listing
  only; object bytes match.
- **Error-recovery byte counts.**  On invalid input pasm2 may emit a 4-byte
  zero placeholder where the clone emits a best-effort instruction size, and
