...
;***********************************************************
        .PABS
        .LOC    100H
        JMP     DRIVER          ; CP/M entry -> main driver
;***********************************************************
; TABLES SECTION
;***********************************************************
START:
...
;***********************************************************
; CP/M BDOS I/O HELPER ROUTINES
;***********************************************************
; These replace the Jove monitor RST 7 console primitives.
; All are fully register preserving: the original macros
; preserved every register (the RST 7 calls did), and many
; callers rely on A (and others) surviving across CARRET,
; PRTLIN, PRTBLK, etc.  Each routine saves and restores
; every register it disturbs, including HL/BC across the
; BDOS call (CALL 5), which preserves nothing per its spec.
;
; BDOS entry: CALL 0005H, function code in C.
;   Func 1 = console input (echo)   -> A
;   Func 2 = console output, E=char
;***********************************************************
;*** OUTPUT <CR><LF> ***
SGCRLF: PUSH    PSW             ; Save all registers
        PUSH    B
        PUSH    D
        PUSH    H
        ...
        POP     PSW
        RET
;*** CLEAR SCREEN (ANSI/VT) ***
SGCLS:  PUSH    PSW             ; Save all registers
        PUSH    B
        PUSH    D
        PUSH    H
        LXI     H,SGANSI        ; ANSI clear-screen sequence
        MVI     B,7             ; Length of the sequence
SGCLS2: MOV     E,M             ; Next byte to output
        MVI     C,2             ; BDOS console output
        PUSH    B               ; Preserve count across BDOS
        ...
        INX     H               ; Advance
        DCR     B               ; Count down
        JNZ     SGCLS2          ; More ? - loop
        POP     H               ; Restore registers
        POP     D
        POP     B
        POP     PSW
        RET
SGANSI: .BYTE   1BH,5BH,48H     ; ESC [ H   (cursor home)
        .BYTE   1BH,5BH,32H,4AH ; ESC [ 2 J (erase screen)
;*** PRINT LINE: BC bytes at HL, then CR/LF ***
SGPLIN: PUSH    PSW             ; Save all registers
        PUSH    B
        PUSH    D
        PUSH    H
        CALL    SGEMIT          ; Emit BC bytes from HL
        MVI     C,2             ; BDOS console output
        MVI     E,0DH           ; Carriage return
        CALL    5
        MVI     C,2             ; BDOS console output
        ...
        POP     PSW
        RET
;*** PRINT BLOCK: BC bytes at HL, no CR/LF ***
SGPBLK: PUSH    PSW             ; Save all registers
        PUSH    B
        PUSH    D
        PUSH    H
        CALL    SGEMIT          ; Emit BC bytes from HL
        ...
        POP     PSW
        RET
;*** EMIT: output BC bytes starting at HL (internal) ***
;    Clobbers A,B,C,D,E,H,L - callers above save them.
SGEMIT: MOV     A,B             ; Is the count zero ?
        ORA     C
        RZ                      ; Yes - nothing to do
SGEM2:  MOV     E,M             ; Next byte to output
        PUSH    B               ; Preserve count across BDOS
        PUSH    H               ;  and string pointer
        MVI     C,2             ; BDOS console output (after save:
                                ;  C is the count low byte, so set
                                ;  the function code only once the
                                ;  count is safely on the stack)
        CALL    5
        POP     H               ; Restore pointer
        POP     B               ; Restore count
...
;                 be marked.
;***********************************************************
DSPBRD: RET                     ; CP/M stub - no memory-mapped video
...
;                 pair HL.
;***********************************************************
INSPCE: RET                     ; CP/M stub - no memory-mapped video
...
;                 register B.
;***********************************************************
BLNKER: RET                     ; CP/M stub - no memory-mapped video
